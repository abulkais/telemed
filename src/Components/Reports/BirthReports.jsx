...
const BirthReports = () => {
  ...
  const handleCreateOrUpdateReport = async () => {
    ...
    try {
      if (reportData.id) {
        ...
        await axios.put(`${baseurl}/api/birthReports/${reportData.id}`, {
          caseId: reportData.caseId,
          doctorId: reportData.doctorId,
          reportDate: reportData.reportDate,
          description: reportData.description,
        });
        toast.success("Birth report updated successfully");
      } else {
        // Create new report
        await axios.post(`${baseurl}/api/birthReports`, {
          caseId: reportData.caseId,
          doctorId: reportData.doctorId,
          reportDate: reportData.reportDate,
          description: reportData.description,
        });
        toast.success("Birth report created successfully");
      }
      fetchReports();
      ...
      toast.error(
        error.response?.data?.error || "Failed to save Birth report"
      );
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await axios.delete(`${baseurl}/api/birthReports/${deleteId}`);
      setReports((prev) => prev.filter((report) => report.id !== deleteId));
      toast.success("Birth report deleted successfully!");
      $("#deleteReport").modal("hide");
    } catch (error) {
      console.error("Delete error:", error);
      toast.error(
        error.response?.data?.error || "Error deleting Birth report!"
      );
    } finally {
      setIsDeleting(false);
    }
    ...
    <div>
      ...
      <div className="doctor-nav-buttons">
        <div className="nav_headings">
          <Link to="/birth-reports" className="doctor-nav-btn active">
      ...
      >
        ...
        >
          <div className="modal-content text-center">
            ...
            <h2>Delete</h2>
            <p>Are you sure you want to delete this Birth report?</p>
