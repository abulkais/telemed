...
const DeathReports = () => {
  ...
  const fetchReports = async () => {
    try {
      const res = await axios.get(`${baseurl}/api/deathReports`);
      setReports(res.data);
    } catch (error) {
      console.error("Error fetching death reports:", error);
      toast.error(
        "Failed to load death reports: " +
  ...
  const fetchCases = async () => {
    try {
      const res = await axios.get(`${baseurl}/api/deathReports/caseIdPatientsDeathReports`);
  ...
  const handleCreateOrUpdateReport = async () => {
    ...
    try {
      if (reportData.id) {
        ...
        await axios.put(`${baseurl}/api/deathReports/${reportData.id}`, {
          caseId: reportData.caseId,
          doctorId: reportData.doctorId,
          reportDate: reportData.reportDate,
          description: reportData.description,
        });
        toast.success("death report updated successfully");
      } else {
        // Create new report
        await axios.post(`${baseurl}/api/deathReports`, {
          caseId: reportData.caseId,
          doctorId: reportData.doctorId,
          reportDate: reportData.reportDate,
          description: reportData.description,
        });
        toast.success("Death report created successfully");
      }
      fetchReports();
      ...
      toast.error(
        error.response?.data?.error || "Failed to save death report"
      );
    }
  };
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await axios.delete(`${baseurl}/api/deathReports/${deleteId}`);
      setReports((prev) => prev.filter((report) => report.id !== deleteId));
      toast.success("death report deleted successfully!");
      $("#deleteReport").modal("hide");
    } catch (error) {
      console.error("Delete error:", error);
      toast.error(
        error.response?.data?.error || "Error deleting death report!"
      );
    } finally {
      setIsDeleting(false);
    }
    ...
    <div>
      ...
      <div className="doctor-nav-buttons">
        <div className="nav_headings">
          ...
          <Link to="/death-reports" className="doctor-nav-btn active">
      ...
      {/* Create/Update death Report Modal */}
      <div
      ...
      >
        ...
        >
          <div className="modal-content text-center">
            ...
            <h2>Delete</h2>
            <p>Are you sure you want to delete this death report?</p>
