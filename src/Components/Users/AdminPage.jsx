...
const Admins = () => {
  ...
  const deleteAdmin = async () => {
    ...
    try {
      await axios.delete(`http://localhost:8080/api/admins/${deleteId}`);
      setAdmins((prev) => prev.filter((item) => item.id !== deleteId));
  ...
  const filteredAdmins = admins.filter(filterAdmins);
  ...
  const downloadExcel = () => {
    const dataToExport = filteredAdmins.map((admin, index) => ({
      "S.N": index + 1,
      "First Name": admin.firstName,
      "Last Name": admin.lastName,
      Email: admin.email,
      Phone: `${admin.phoneCountryCode || "+91"} ${admin.phoneNumber}`,
      Gender: admin.gender,
      Status: admin.status,
      "Date Of Birth": admin.dateOfBirth,
    }));
  ...
  return (
    <div>
      ...
      <div className="nav_headings">
          <Link to="/admin" className="doctor-nav-btn active">
              ...
              currentItems.map((admin, index) => (
                <tr key={admin.id}>
                  ...
                  <td>
                    ...
                    >
                      <button className="btn" onClick={() => handleView(admin)}>
                        <VisibilityIcon className="text-info" />
                      </button>
                      <button className="btn" onClick={() => handleEdit(admin)}>
