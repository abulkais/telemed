...
const Nurses = () => {
  ...
  const fetchNurses = async () => {
    try {
      const res = await axios.get("http://localhost:8080/api/nurses");
  ...
  const deleteNurse = async () => {
    ...
    try {
      await axios.delete(`http://localhost:8080/api/nurses/${deleteId}`);
      setNurses((prev) => prev.filter((item) => item.id !== deleteId));
      toast.success("Nurse deleted successfully!");
      $("#deleteNurse").modal("hide");
    } catch (error) {
  ...
  const handleView = (nurse) => {
    navigate(`/nurses/${nurse.id}/view`);
  };

  const handleEdit = (nurse) => {
    navigate(`/nurses/${nurse.id}/edit`);
  };

  const filterNurses = (nurse) => {
    ...
    const matchesStatus =
      statusFilter === "ALL" ||
      (statusFilter === "ACTIVE" && nurse.status === "Active") ||
      (statusFilter === "INACTIVE" && nurse.status === "Inactive");

    return matchesSearch && matchesStatus;
  };

  const filteredNurses = nurses.filter(filterNurses);

  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = filteredNurses.slice(indexOfFirstItem, indexOfLastItem);
  ...
  const downloadExcel = () => {
    const dataToExport = filteredNurses.map((nurse, index) => ({
      "S.N": index + 1,
      "First Name": nurse.firstName,
      "Last Name": nurse.lastName,
      Email: nurse.email,
      Phone: `${nurse.phoneCountryCode || "+62"} ${nurse.phoneNumber}`,
      "Blood Group": nurse.bloodGroups,
      Designation: nurse.designation,
      Qualification: nurse.qualification,
      "Date Of Birth": nurse.dateOfBirth,
      Gender: nurse.gender,
      Status: nurse.status,
    ...
    XLSX.writeFile(workbook, "Nurses_List.xlsx");
  };
    ...
    <div>
      ...
      <div className="doctor-nav-buttons">
        <div className="nav_headings">
          ...
          <Link to="/nurses" className="doctor-nav-btn active">
      ...
      <div className="custom-table-responsive">
              ...
              currentItems.map((nurse, index) => (
                <tr key={nurse.id}>
                  ...
                  <td>
                    <div className="d-flex align-items-center">
                      ...
                      <div className="flex-wrap">
                        ...
                        <p className="mb-0">{nurse.email}</p>
                      </div>
                    </div>
                  </td>
                  <td>{`${nurse.phoneCountryCode || "+62"} ${
                    nurse.phoneNumber
                  }`}</td>
                  ...
                  <td>
                    ...
                    >
                      <button className="btn" onClick={() => handleView(nurse)}>
                        <VisibilityIcon className="text-info" />
                      </button>
                      <button className="btn" onClick={() => handleEdit(nurse)}>
                        <i className="text-primary fa fa-edit fa-lg" />
                      </button>
                      <button
                        className="btn"
                        data-toggle="modal"
                        data-target="#deleteNurse"
        ...
        <div className="d-flex justify-content-between align-items-center mt-5">
          <div>
            ...
            {Math.min(indexOfLastItem, filteredNurses.length)} of{" "}
            {filteredNurses.length} results
          </div>
          <nav>
