...
const Receptionists = () => {
  ...
  const fetchReceptionists = async () => {
    try {
      const res = await axios.get("http://localhost:8080/api/receptionists");
  ...
  const deleteReceptionist = async () => {
    if (!deleteId) {
      toast.error("Invalid Receptionist ID!");
      return;
    }
    ...
    try {
      await axios.delete(`http://localhost:8080/api/receptionists/${deleteId}`);
      setReceptionists((prev) => prev.filter((item) => item.id !== deleteId));
      toast.success("Receptionist deleted successfully!");
      $("#deleteReceptionist").modal("hide");
    } catch (error) {
  ...
  const handleView = (receptionist) => {
    navigate(`/receptionists/${receptionist.id}/view`);
  };
  ...
  const filterReceptionists = (receptionist) => {
    ...
    const matchesStatus =
      ...
      (statusFilter === "INACTIVE" && receptionist.status === "Inactive");

    return matchesSearch && matchesStatus;
  };

  const filteredReceptionists = receptionists.filter(filterReceptionists);

  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = filteredReceptionists.slice(
    indexOfFirstItem,
    indexOfLastItem
  );
  ...
  const downloadExcel = () => {
    const dataToExport = filteredReceptionists.map((receptionist, index) => ({
      "S.N": index + 1,
      "First Name": receptionist.firstName,
      "Last Name": receptionist.lastName,
      Email: receptionist.email,
      Phone: `${receptionist.phoneCountryCode || "+62"} ${
        receptionist.phoneNumber
      }`,
      "Blood Group": receptionist.bloodGroup,
      Designation: receptionist.designation,
      Qualification: receptionist.qualification,
      "Date Of Birth": receptionist.dateOfBirth,
    ...
    XLSX.writeFile(workbook, "Receptionists_List.xlsx");
  };
  ...
  return (
    <div>
      ...
      <div className="nav_headings">
          ...
          <Link to="/receptionists" className="doctor-nav-btn active">
      ...
      <div className="custom-table-responsive">
        <table className="table custom-table-striped custom-table table-hover text-center">
          ...
          <tbody>
            {currentItems.length > 0 ? (
              currentItems.map((receptionist, index) => (
                  ...
                  <td>
                    <div className="d-flex align-items-center">
                      ...
                      <div className="flex-wrap">
                        ...
                        <p className="mb-0">{receptionist.email}</p>
                      </div>
                    </div>
                  </td>
                  <td>{`${receptionist.phoneCountryCode || "+62"} ${
                    receptionist.phoneNumber
                  }`}</td>
                  ...
                  <td>
                    ...
                    >
                      ...
                      <button
                        className="btn"
                        data-toggle="modal"
                        data-target="#deleteReceptionist"
        ...
        <div className="d-flex justify-content-between align-items-center mt-5">
          <div>
            ...
            {Math.min(indexOfLastItem, filteredReceptionists.length)} of{" "}
            {filteredReceptionists.length} results
          </div>
          <nav>
      ...
      <div
        ...
        id="deleteReceptionist"
        tabIndex="-1"
        role="dialog"
        aria-labelledby="deleteReceptionist"
        aria-hidden="true"
      >
        <div
